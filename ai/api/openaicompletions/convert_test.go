...
func TestBuildParamsKimiK3DetectedFromExternalEndpoint(t *testing.T) {
	prompt := ai.Prompt{Messages: []ai.Message{userText("hi")}}
	opts := ai.StreamOptions{
	...
	kimi := ai.Provider{Name: "customer-defined-name", BaseURL: "https://api.moonshot.cn/v1/"}
	model := ai.Model{Provider: "customer-defined-name", ID: "kimi-k3", Reasoning: true, ToolCall: true}
